...
def process_lines(lines):
    """
    处理行内容，确保?a=view 的处理方式与脚本 1 一致（替换为?d=true）
    :param lines: 解码后的行列表
    :return: 处理后的行列表
    """
    processed_lines = []
...
def main():
    ...
    address_format_prompt += "例如：[ANi] 火影忍者 - 01 [1080P][Baha][WEB-DL][AAC AVC][CHT].mp4\n"
    address = input(address_format_prompt)
    address = process_url(address)

    num_lines_prompt = "请输入需要生成的行数："
    ...
    choice_prompt = "是否生成.strm 文件？(y/n)：生成.strm 文件可以用于[简要说明生成.strm文件的作用和用途]\n"
